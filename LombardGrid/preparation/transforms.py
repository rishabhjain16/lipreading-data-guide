...
"""Lombard GRID - Text Transform with shared SentencePiece tokenizer.

This mirrors the GRID implementation and uses the repo-wide SPM model:

- spm/unigram/unigram5000.model
- spm/unigram/unigram5000_units.txt

Important: the shared model is trained with an uppercase vocabulary, so we
normalize input text with `.upper()` before encoding.
"""
