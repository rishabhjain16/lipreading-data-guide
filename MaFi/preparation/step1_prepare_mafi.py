#!/usr/bin/env python3
"""
MaFi Dataset Preprocessing Pipeline (RetinaFace)
================================================

Preprocessing script for MaFi (Massive Faces) dataset - a silent video word guessing dataset.
Uses RetinaFace detector for face detection and lip/face cropping.

Usage:
    python step1_prepare_mafi.py \
        --data-dir "/media/rishabhjain/SSD/MaFi/Videos Folder" \
        --root-dir /path/to/output \
        --crop-type lips

Note: MaFi videos are silent (no audio). Transcripts are extracted from filenames.
"""

import argparse
import math
import os
import warnings
from pathlib import Path

import torch
from data.data_module import AVSRDataLoader
from tqdm import tqdm
from utils import save_vid_txt
import pandas as pd

warnings.filterwarnings("ignore")

# Argument parsing
parser = argparse.ArgumentParser(description="MaFi Dataset Preprocessing with RetinaFace")
parser.add_argument(
    "--data-dir",
    type=str,
    required=True,
    help="Directory where MaFi 'Videos Folder' is stored",
)
parser.add_argument(
    "--root-dir",
    type=str,
    required=True,
    help="Root directory of preprocessed dataset",
)
parser.add_argument(
    "--detector",
    type=str,
    default="retinaface",
    help="Type of face detector (default: retinaface)",
)
parser.add_argument(
    "--crop-type",
    type=str,
    default="lips",
    choices=["lips", "face"],
    help="Crop type: lips (96x96, mouth region) or face (224x224, full face)",
)
parser.add_argument(
    "--groups",
    type=int,
    default=1,
    help="Number of threads to be used in parallel",
)
parser.add_argument(
    "--job-index",
    type=int,
    default=0,
    help="Index to identify separate jobs (useful for parallel processing)",
)
args = parser.parse_args()

# Setup output directories based on crop type
if args.crop_type == "face":
    dst_vid_dir = os.path.join(args.root_dir, "mafi_video_face_224x224")
    dst_txt_dir = os.path.join(args.root_dir, "mafi_text_face_224x224")
    csv_suffix = "_face_224x224"
else:
    dst_vid_dir = os.path.join(args.root_dir, "mafi_video")
    dst_txt_dir = os.path.join(args.root_dir, "mafi_text")
    csv_suffix = ""

labels_dir = os.path.join(args.root_dir, "labels")
os.makedirs(labels_dir, exist_ok=True)

print(f"Output directories:")
print(f"  Videos: {dst_vid_dir}")
print(f"  Text: {dst_txt_dir}")
print(f"  Labels: {labels_dir}")

# Initialize video dataloader with RetinaFace
if args.detector == "retinaface":
    from detectors.retinaface.detector import LandmarksDetector
    from detectors.retinaface.video_process import VideoProcess
    
    landmarks_detector = LandmarksDetector(device="cuda:0" if torch.cuda.is_available() else "cpu")
    
    # Get the absolute path to the mean face file
    script_dir = Path(__file__).parent
    mean_face_path = script_dir / "detectors" / "retinaface" / "20words_mean_face.npy"
    
    if not mean_face_path.exists():
        print(f"❌ Error: Mean face file not found at {mean_face_path}")
        exit(1)
    
    if args.crop_type == "face":
        video_process = VideoProcess(
            mean_face_path=str(mean_face_path),
            crop_width=224,
            crop_height=224,
            start_idx=48,
            stop_idx=68,
            window_margin=12,
            convert_gray=False,  # Keep RGB for video output
        )
    else:  # lips
        video_process = VideoProcess(
            mean_face_path=str(mean_face_path),
            crop_width=96,
            crop_height=96,
            start_idx=48,
            stop_idx=68,
            window_margin=12,
            convert_gray=False,  # Keep RGB for video output
        )
    
    class CustomAVSRDataLoader:
        def __init__(self):
            self.landmarks_detector = landmarks_detector
            self.video_process = video_process
        
        def load_data(self, data_filename):
            import torchvision
            video = torchvision.io.read_video(data_filename, pts_unit="sec")[0].numpy()
            landmarks = self.landmarks_detector(video)
            video = self.video_process(video, landmarks)
            if video is None:
                raise TypeError("video cannot be None")
            return torch.tensor(video)
    
    vid_dataloader = CustomAVSRDataLoader()
else:
    vid_dataloader = AVSRDataLoader(
        modality="video", detector=args.detector, convert_gray=False
    )

def extract_word_from_filename(filename):
    """
    Extract the word from MaFi filename.
    Handles both simple names (e.g., 'big.mp4') and complex names (e.g., 'cauliflower_clear_small_muted.mp4')
    """
    stem = Path(filename).stem
    # Remove common suffixes from A1 dataset
    for suffix in ['_clear_small_muted', '_clear_small', '_muted', '_clear']:
        if stem.endswith(suffix):
            stem = stem[:-len(suffix)]
            break
    return stem

def find_video_files(data_dir):
    """Find all video files organized by speaker"""
    data_path = Path(data_dir)
    speakers = ['A1', 'A2', 'B1', 'B2', 'B3']
    
    video_files = {}
    for speaker in speakers:
        speaker_path = data_path / speaker
        if not speaker_path.exists():
            print(f"Warning: Speaker directory {speaker} not found")
            continue
        
        # Find all mp4 files in speaker directory
        videos = list(speaker_path.rglob("*.mp4"))
        video_files[speaker] = sorted(videos)
        print(f"Found {len(videos)} videos for speaker {speaker}")
    
    return video_files

# Find all video files
print(f"\nScanning MaFi dataset at: {args.data_dir}")
video_files_by_speaker = find_video_files(args.data_dir)

# Flatten to single list for processing
all_filenames = []
for speaker, videos in video_files_by_speaker.items():
    for video_path in videos:
        all_filenames.append((speaker, str(video_path)))

if not all_filenames:
    print("❌ Error: No video files found")
    exit(1)

print(f"\nTotal videos to process: {len(all_filenames)}")

# Split work for parallel processing
unit = math.ceil(len(all_filenames) / args.groups)
files_to_process = all_filenames[args.job_index * unit : (args.job_index + 1) * unit]

print(f"Processing {len(files_to_process)} files (job {args.job_index + 1}/{args.groups})")

# Initialize CSV data collection for all speakers + combined
csv_data_all = []
csv_data_by_speaker = {speaker: [] for speaker in ['A1', 'A2', 'B1', 'B2', 'B3']}

processed_count = 0
skipped_count = 0

for speaker, vid_filename in tqdm(files_to_process, desc="Processing videos"):
    vid_path = Path(vid_filename)
    
    # Extract word from filename
    word = extract_word_from_filename(vid_path.name)
    
    try:
        video_data = vid_dataloader.load_data(vid_filename)
    except (UnboundLocalError, TypeError, OverflowError, AssertionError, Exception) as e:
        skipped_count += 1
        continue
    
    if video_data is None:
        skipped_count += 1
        continue
    
    # Create output filename: speaker/word.mp4
    dst_vid_filename = os.path.join(dst_vid_dir, speaker, f"{word}.mp4")
    dst_txt_filename = os.path.join(dst_txt_dir, speaker, f"{word}.txt")
    
    # Use the whole video (MaFi videos are short single-word utterances)
    video_length = len(video_data)
    
    # Basic quality check
    if video_length < 5:  # At least 5 frames
        skipped_count += 1
        continue
    
    # Save video and text (no audio for MaFi)
    save_vid_txt(
        dst_vid_filename,
        dst_txt_filename,
        video_data,
        word,
    )
    
    # Add to CSV data
    csv_entry = {
        'speaker': speaker,
        'video_path': dst_vid_filename,
        'text_path': dst_txt_filename,
        'word': word,
        'num_frames': video_length,
        'detector': args.detector,
        'crop_type': args.crop_type,
    }
    
    csv_data_all.append(csv_entry)
    csv_data_by_speaker[speaker].append(csv_entry)
    processed_count += 1

print(f"\n✅ Processing complete!")
print(f"   Processed: {processed_count} videos")
print(f"   Skipped: {skipped_count} videos")

# Save CSV files
if csv_data_all:
    # Combined CSV
    df_all = pd.DataFrame(csv_data_all)
    csv_all_path = os.path.join(labels_dir, f"mafi_all{csv_suffix}.csv")
    df_all.to_csv(csv_all_path, index=False)
    print(f"\n📊 Saved combined CSV: {csv_all_path} ({len(df_all)} entries)")
    
    # Individual speaker CSVs
    for speaker, data in csv_data_by_speaker.items():
        if data:
            df_speaker = pd.DataFrame(data)
            csv_speaker_path = os.path.join(labels_dir, f"mafi_{speaker}{csv_suffix}.csv")
            df_speaker.to_csv(csv_speaker_path, index=False)
            print(f"   Saved {speaker} CSV: {csv_speaker_path} ({len(df_speaker)} entries)")

print("\n🎉 MaFi preprocessing complete!")
