...
def explore_video_files(data_dir):
    ...
    if all_videos:
        ...
        print(f"\n🔍 Analyzing video properties (sampling {min(10, len(all_videos))} videos)...")
        analyze_video_properties(all_videos[:10], data_path)
...
def analyze_video_properties(video_files, data_path):
    ...
    if total_duration > 0:
        ...
        print(f"⏱️  Total duration (sampled): {total_duration:.1f}s ({hours:.2f} hours)")
        
        if len(video_files) < 100:  # Only extrapolate for small samples
            estimated_total_hours = hours * (13926 / len(video_files))  # Based on your 13926 total files
            print(f"📈 Estimated total dataset duration: ~{estimated_total_hours:.1f} hours")
    ...
    if file_sizes:
        ...
        print(f"💾 Sample total size: {total_size_gb:.2f} GB")
        
        if len(video_files) < 100:
            estimated_total_gb = total_size_gb * (13926 / len(video_files))
            print(f"📈 Estimated dataset size: ~{estimated_total_gb:.1f} GB")
